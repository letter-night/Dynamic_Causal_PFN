"""
CODE ADAPTED FROM: https://github.com/toonvds/TESAR-CDE/ &
...
https://github.com/toonvds/TESAR-CDE/blob/main/src/utils/cancer_simulation.py
...
def simulate(simulation_params, assigned_actions=None, intensity_cov_only=False, max_intensity=1, noise_level=0.01):
    """
    ...
    noise_terms = np.random.normal(0, noise_level, (num_patients, num_time_steps))
    ...
    for i in range(num_patients):
        ...
        for t in range(0, num_time_steps):
            ...
            if intensity_cov_only:
                ...
                obs_prob = np.min((np.float64(1.), max_intensity / (1.0 + np.exp(
...
def get_cancer_sim_data(
    ...
    model_root="results",
    window_size=15,
    noise_level=0.01 # noise level for test data
):
    if window_size == 15:
...
def make_last_counterfactual(treatment):
    if tuple(treatment) == (0,0):
        return np.array((1,1))
    if tuple(treatment) in [(1,0), (0,1), (1,1)]:
        return np.array((0,0))