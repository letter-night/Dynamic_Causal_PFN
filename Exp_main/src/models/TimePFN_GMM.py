...
class GMMHead(nn.Module):
	...
	def forward(self, z:torch.Tensor):
		...
		mu = self.fc_mu(z)
		sigma = F.softplus(self.fc_sigma(z)) + 1e-3 
		return pi, mu, sigma 
	


class TimePFN(pl.LightningModule):


	def __init__(self, 
			  seq_len, pred_len, gmm_n_components, gmm_min_sigma, gmm_pi_temp,
			  c_out, patch_size, embed_dim, dropout, n_heads, d_ff, activation, e_layers,
			  d_model, treatment_size, prediction_window, interpolation_method, learning_rate,
		...
		self.gmm_min_sigma=gmm_min_sigma
		self.pi_temp = gmm_pi_temp 

		self.z_dim = c_out + treatment_size
		...
		# Embedding
		self.enc_embedding = DataEmbedding_FeaturePatching(self.seq_len, patch_size,
		...
		self.encoder = Encoder(
			[
				EncoderLayer(
					...
					output_attention=False), embed_dim, n_heads),
					embed_dim,
					d_ff,
					dropout=dropout,
		...
		self.projector1 = nn.Linear(((seq_len-patch_size)//(patch_size//2)+1) * (embed_dim), d_model, bias=True)
		self.non_linear = nn.GELU()
		self.projector2 = nn.Linear(d_model, pred_len, bias=True)

		self.gmm_head = GMMHead(
			z_dim=self.z_dim, 
			n_components=self.gmm_n_components,
			min_sigma=self.gmm_min_sigma,
	...
	def _common_step(self, batch):
		...
		x_coeffs = torchcde.linear_interpolation_coeffs(x)
		if torch.isnan(x_coeffs).any():
			print('NaN detected in input x!')

		# data_X: batch, seq_len (55), channels (7)
		# data_tr: batch, seq_len (55), channels (4)
		# data_y: batch, seq_len (55), steps (5)
		# data_tr_tau: batch, seq_len (60), channels (4)
		# data_ae: batch, seq_len (55), steps (5)
		...
		# [batch, seq_len (55), channels (1)]
		# [batch, seq_len (55)]

		# Multistep treatment assignments
		in_time = x.shape[1]-1 # 54
		# for tau-step ahead, need grid of tau+1 treatment assignments
		tr_tau = tr_tau[:, (in_time):(in_time+self.tau), :]
		# [batch, prediction_window, treatment_size]: [batch, 1, 4]


		B, _, N = x_coeffs.shape # B L N

		enc_out = self.enc_embedding(x_coeffs)
		...
		# batch, covariates, emb_dim 

		pred_y = self.projector2(dec_out)
		# [batch, covarates, pred_len]

		pred_y = pred_y.permute(0, 2, 1)
		# [batch, pred_len, covariates]

		concat = torch.concatenate((pred_y, tr_tau), dim=2)
		# [batch, pred_len, covariates (7+4=11)]

		pi, mu, sigma = self.gmm_head(concat)

		pi, mu, sigma = pi[:, -1, :], mu[:, -1, :], sigma[:, -1, :]
		pi, mu, sigma = pi.squeeze(1), mu.squeeze(1), sigma.squeeze(1)
		# [batch, num_gmm_components]
		...
		# log_norm_const = 0.5 * torch.log(torch.tensor(2.0 * torch.pi, device=y.device, dtype=y.dtype))
		# z = (y[:, None] - mu) / sigma 
		# [batch, num_gmm_components]

		mask = (active_entries > 0) & (~torch.isnan(y))

		if mask.any():
			clean_y = y[mask]
			clean_pi = pi[mask]
			clean_mu = mu[mask]
			clean_sigma = sigma[mask]

			mix = D.Categorical(probs=clean_pi)
			comp = D.Normal(loc=clean_mu, scale=clean_sigma)
			gmm = D.MixtureSameFamily(mix, comp)

			loss = -gmm.log_prob(clean_y).mean()
		else:
			loss=mu.sum() * 0

		# log_prob = -0.5 * z**2 - torch.log(sigma) - log_norm_const
		# log_mix = torch.logsumexp(torch.log(pi) + log_prob, dim=-1) 
		# log_mix = torch.where(active_entries > 0, log_mix, torch.zeros_like(log_mix))

		# loss = -log_mix.sum() / (active_entries.sum().clamp_min(1.0))

		pi, mu, sigma = pi.unsqueeze(-1).permute(1, 0, 2), mu.unsqueeze(-1).permute(1, 0, 2), sigma.unsqueeze(-1).permute(1, 0, 2)
		# [num_gmm_components, batch, 1]
		...
		# 	pi * (sigma**2 + mu**2), dim=1, keepdim=False 
		# ) # [batch]
		# print(second_moment.shape)
		# var = second_moment - y_pred**2 # [batch]
		# print(var.shape)
		...
		return loss, pi, mu, sigma, y, x 


	def training_step(self, batch):
		loss, pi, mu_pred, sigma_pred, y, _ = self._common_step(batch)
		self.log_dict(
			{"train_loss":loss},
			prog_bar=True,
			on_step=False,
		...
		return {"loss":loss, "pi_pred":pi, "y_pred":mu_pred, "sigma_pred":sigma_pred, "y":y}
	
	def validation_step(self, batch, batch_idx):
		loss, pi, mu_pred, sigma_pred, y, _ = self._common_step(batch)

		self.coverage_metric(torch.concatenate((pi, mu_pred, sigma_pred), dim=-1), y.squeeze(dim=-1))
	...
	def test_step(self, batch, batch_idx):
		loss, pi, mu_pred, sigma_pred, y, _ = self._common_step(batch)
		self.coverage_metric(torch.concatenate((pi, mu_pred, sigma_pred), dim=-1),
					   y.squeeze(dim=-1))
		size = self.coverage_metric.size()
		coverage = self.coverage_metric.compute()
		self.log_dict({"loss":loss, "coverage":coverage, "size":size}, prog_var=True, logger=True)

		return {"loss":loss, "pi":pi, "y_pred":mu_pred, "sigma_pred":sigma_pred, "y":y, 
		  "coverage":coverage, "size":size}
	
	def predict_step(self, batch, batch_idx, dataloader_idx=None):
		loss, pi, mu_pred, sigma_pred, y, x = self._common_step(batch)
		self.coverage_metric(torch.concatenate((pi, mu_pred, sigma_pred), dim=-1),
					   y.squeeze(dim=-1))
		size = self.coverage_metric.size()
		coverage = self.coverage_metric.compute()

		return {"loss":loss, "pi":pi, "y_pred":mu_pred, "sigma_pred":sigma_pred, "y":y, "x":x,
		  "coverage":coverage, "size":size}
	
	def configure_optimizers(self):
		
		params = [self.enc_embedding.parameters(),
			self.encoder.parameters(),
			self.projector1.parameters(),
			self.projector2.parameters(),
			self.gmm_head.parameters(),
			]
		learning_rates = [self.learning_rate, self.learning_rate, self.learning_rate, self.learning_rate, self.learning_rate]
