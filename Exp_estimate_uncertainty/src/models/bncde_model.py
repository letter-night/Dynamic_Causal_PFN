...
class BNCDE(pl.LightningModule):
    ...
    def _common_step(self, batch):
        ...
        # x [batch, seq_len (55), channels (7)]
        # y [batch, seq_len (55), channels (5)]
        # tr_tau [batch, seq_len (60), channels (4)]
        # active entries [batch, seq_len (55), channels (5)]
        ...
        # encoder output z1: mc_samples, batch, 1, hidden_size
        # decoder input tr_tau: batch, 1, 4
        # decoder input z1: mc_samples, batch, 1, hidden_size
        # decoder output z1: mc_samples, batch, 1, hidden_size

        # Prediction
        pred = self.pred_head(z1)
        y_pred, sigma_pred = pred[:, :, :, 0], F.softplus(pred[:, :, :, 1])
        # prediction head output: mc_samples, batch, 1, 2
        # y_pred, sigma_pred: [mc_samples, batch, 1], [mc_samples, batch, 1]
