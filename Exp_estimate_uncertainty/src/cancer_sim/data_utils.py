...
def process_data(pickle_map):
    ...
    # get final treatment and intensity scaling values
    scale_intensity = training_processed['intensity'].copy().mean(), training_processed['intensity'].copy().std()
    scale_tr = training_processed["current_treatments"].copy().mean(axis=(0,1)), training_processed["current_treatments"].copy().std(axis=(0,1))

    return training_processed, validation_processed, test_f_processed, test_cf_processed,\
        {'scale_intensity': scale_intensity, 'scale_tr':scale_tr}
...
def data_to_torch_tensor(data, sample_prop=1, time_concat=-1):
    ...
    data_intensity = data['intensity'][:, :-5]
    ...
    # add intensity as in TE-CDE, do not use obs prob as in TESAR-CDE
    data_X = torch.from_numpy(np.concatenate((data_x, data_intensity[..., None]), axis=-1))
...
def intensity_and_treatment_scale(data, scale):
    with np.errstate(divide='ignore', invalid='ignore'):
        data['intensity'] = (data['intensity'] - scale['scale_intensity'][0]) /  scale['scale_intensity'][1]
        data['current_treatments'] = np.nan_to_num((data['current_treatments'] - scale['scale_tr'][0]) / scale['scale_tr'][1])
        data['future_treatments'] = np.nan_to_num((data['future_treatments'] - scale['scale_tr'][0]) / scale['scale_tr'][1])
