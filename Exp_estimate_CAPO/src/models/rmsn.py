...
class RMSNPropensityNetworkHistory(RMSN):
    ...
    def forward(self, batch, detach_treatment=False):
        ...
        vitals_or_prev_outputs = forward_fill(vitals_or_prev_outputs).to(self.device)
...
class RMSNDecoder(RMSN):
    ...
    def forward(self, batch, detach_treatment=False):
        ...
        observation_mask = (~torch.isnan(prev_outputs)).cumsum(dim=1) / prev_outputs.size(1)
        prev_outputs = forward_fill(prev_outputs).to(self.device)
        static_features = batch['static_features']
