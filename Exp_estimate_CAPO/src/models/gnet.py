...
class GNet(TimeVaryingCausalModel):
    ...
    def forward(self, batch, sample=False):
        ...
        vitals = forward_fill(batch['vitals']).to(self.device) if self.has_vitals else None
        prev_outputs = torch.cat((forward_fill(batch['prev_outputs']).to(self.device),
                            (~torch.isnan(batch['prev_outputs'])).cumsum(dim=1) /batch['prev_outputs'].size(1)), dim=-1)
    ...
    def training_step(self, batch, batch_ind):
        ...
        if self.has_vitals: # avoid weird inplace bug
            next_vitals_pred = next_vitals_pred[:, :-1, :].clone()
            next_vitals_pred[torch.isnan(batch['next_vitals'])] = 0
            next_vitals = batch['next_vitals'].clone()
            next_vitals[torch.isnan(batch['next_vitals'])] = 0
            vitals_mse_loss = F.mse_loss(next_vitals_pred, next_vitals, reduce=False)
        else:
            vitals_mse_loss = 0
