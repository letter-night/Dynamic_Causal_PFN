...
from src.data.cancer_sim.process_irregular_data import apply_observation_mask
...
class SyntheticCancerDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.params = generate_params(num_patients, chemo_coeff=chemo_coeff, radio_coeff=radio_coeff, window_size=window_size,
                                      lag=lag, obs_coeff=obs_coeff)
        self.subset_name = subset_name

        if mode == 'factual':
            self.data = simulate_factual(self.params, seq_length, max_intensity=max_intensity)
        ...
        self.data = apply_observation_mask(self.data, apply_mask=True if mode == 'factual' else False)
    ...
    def process_data(self, scaling_params):
        ...
        if not self.processed:
            ...
            treatment_times = (treatments.sum(axis=-1, keepdims=True)>0)*1.
            ...
            self.data['prev_treatment_times'] = np.concatenate((np.zeros((treatment_times.shape[0], 1, 1)),
                                                                treatment_times[:, :-1, :]), axis=1)
            self.data['current_treatment_times'] = treatment_times
...
class SyntheticCancerDatasetCollection(SyntheticDatasetCollection):
    ...
    def __init__(self,
        ...
        Args:
            ...
            seed: Random seed
            lag: Lag for treatment assignment window
            obs_coeff: Observation coefficient
            max_intensity: Maximum intensity of observations (<=1)
            cf_seq_mode: sliding_treatment / random_trajectories / fixed_treatment
            treatment_mode: multiclass / multilabel
            treatment_sequence: nested list of treatments to train on (only for fixed_treatment)
        """
        ...
        self.train_f = SyntheticCancerDataset(chemo_coeff, radio_coeff, num_patients['train'], window_size, max_seq_length,
                                              'train', lag=lag,
                                              obs_coeff = obs_coeff, max_intensity = max_intensity,
