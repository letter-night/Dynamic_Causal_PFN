...
def generate_params(num_patients, chemo_coeff, radio_coeff, window_size, lag, obs_coeff):
    """
    ...
    basic_params["obs_sigmoid_betas"] = np.array(
        [obs_coeff / 2.0 for i in patient_types],
    )
    basic_params["obs_coeff"] = obs_coeff
...
def simulate_factual(simulation_params, seq_length, assigned_actions=None, max_intensity=1.):
    """
    ...
    obs_probabilities = np.zeros((num_patients, seq_length))
...
def get_scaling_params(sim):
    ...
    for k in real_idx:
        ...
        means[k] = np.nanmean(active_values)
        stds[k] = np.nanstd(active_values)

    # Add means for static variables`
    means['patient_types'] = np.nanmean(sim['patient_types'])
    stds['patient_types'] = np.nanstd(sim['patient_types'])
...
if __name__ == "__main__":
    ...
    params = generate_params(num_patients, chemo_coeff=chemo_coeff, radio_coeff=radio_coeff, lag=lag, obs_coeff=obs_coeff)
    params['window_size'] = window_size
    training_data = simulate_factual(params, seq_length)

    params = generate_params(int(num_patients / 10), chemo_coeff=chemo_coeff, radio_coeff=radio_coeff, lag=lag, obs_coeff=obs_coeff)
    params['window_size'] = window_size
