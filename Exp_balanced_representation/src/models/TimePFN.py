...
class TimePFN(nn.Module):
	...
	def __init__(self, configs):
		...
		# Embedding
		self.enc_embedding = DataEmbedding_FeaturePatching(configs.seq_len, configs.patch_size,
													 embed_dim=configs.embed_dim, dropout=0.1)
		...
		# projector2 -> GMM Head change!!
		self.projector3 = nn.Linear(configs.d_model, 4, bias=True)
	...
	def forecast(self, x_enc):
		...
		pred_y = self.projector2(dec_out)[:, 0, :]
		# projector 2 -> GHH Head change!!
		treatment = self.projector3(dec_out)[:, 0, :]

		return pred_y, treatment
	

	def forward(self, x_enc, mask=None):
		pred_y, treatment = self.forecast(x_enc)
		return pred_y * 1150, treatment
